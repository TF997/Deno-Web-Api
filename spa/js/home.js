...
function loadCustomerData(node, page, limit, offset){
	const jobs = JSON.parse(localStorage.getItem(`customerHomeJobs${page}`))
	let fragment;
	...
	if(pages.remaining == 0 ){
		...
		buttonFragment.querySelector('button[name="next"]').addEventListener("click", async function() {
			loadPage('home', page+1, limit, offset+limit)
		});
	}
	if(pages.number < 2 ){
		console.log('Prev')
...
async function loadCustomerDataIfNew(node, page, limit, offset){
	const etag = localStorage.getItem(`customerHomeEtag${page}`)
	let fragment;
	...
	if(jobs != '304' && jobs != '404'){
		console.log('200')
		loadPage('home', page, limit, offset)
	}
}
...
async function getUserJobs(etag, page, limit, offset){
	...
	var url = `/api/v1/jobs?username=${username}&withAppliances=true&limit=${limit}&offset=${offset}`
	...
	if(response.status == '200'){
		...
		localStorage.setItem(`customerHomeEtag${page}`, responseEtag)
		console.log(json.data)
		const data = json.data
		localStorage.setItem(`customerHomeJobs${page}`, JSON.stringify(data))
		return('200')
