...
export function showMessage(message, delay = 3000) {
	...
	document.querySelector('aside').className = "terminal-alert terminal-alert-error"
	setTimeout( () => document.querySelector('aside').className = ('hidden'), delay)
}

/* NAV FUNCTIONS */

export function loadPage(page, pageNum=1, limit=20, offset=0) {
	console.log(`PAGE ${pageNum} ${limit} ${offset	}`)
	history.pushState(null, null, `/${page}`)
	triggerPageChange(pageNum, limit, offset)
}

export async function unauthorizedLogout(){
	localStorage.clear()
	loadPage('login')
	showMessage('Your session has either expired or is invalid please login')
}

export async function triggerPageChange(pageNum=1, limit=20, offset=0) {
	console.log(`PAGE ${pageNum} ${limit} ${offset	}`)
	...
	try {
		...
		await module.setup(node, pageNum, limit, offset) // the setup script may need to modify the template fragment before it is displayed
	} catch(err) {
