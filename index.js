...
import { extractCredentials, getEtag, setHeaders, extractToken, logAPICall } from "./api/modules/util.js";
import { verifyToken } from "./api/modules/encryption.js";
...
app.use(async (context, next) => {
	...
	if(context.request.url.pathname.includes("/api/")){
		setHeaders(context)
		await logAPICall(context.request)
	}
	// if the call is to the API it must include the correct Content-Type
	if(context.request.url.pathname.includes("/api/") && !(context.request.url.pathname.includes("/api/v1/accounts") && context.request.method === 'POST')) {
		...
		if(!context.request.url.pathname.includes('/accounts')) {
			...
			try {
				console.log(token)
				const jwt = await extractToken(token)
